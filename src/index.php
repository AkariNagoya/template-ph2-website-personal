...
require 'dbconnect.php';
  ...
  // $tbl2 = $dbh->query($sql2);
  // $tbl2->setFetchMode(PDO::FETCH_ASSOC);
  // foreach($tbl2 as $row){
  //   echo'<pre>';
  //   print_r($row);
  //   echo '</pre>';
  ...
  //   $question["choices"] = [];
  //   foreach($choices as $qKey => $choice){
    ...
    <div class="p-quiz-container l-container">
      <section class="p-quiz-box js-quiz" data-quiz="0">
        <div class="p-quiz-box__question">
          ...
          <div>
            <img src='./assets/img/quiz/<?= $questions[$i] ["image"] ?>'>
          </div>
          <ul class="p-quiz-box__answer__list">
            <?php foreach($questions[$i]["choices"] as $index => $answer){ ?>
              <li class="p-quiz-box__answer__item">
                <button class="p-quiz-box__answer__button js-answer" data-answer="<?= $answer["valid"] ?>">
                  <?= $answer['name'] ?><i class="u-icon__arrow"></i>
                </button>
              </li>
              <?php } ?>
              <?php
              if ($questions[$i] ["supplement"] !== null){
                echo
